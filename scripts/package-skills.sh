...
for dir in skills/*/; do
  name=$(basename "$dir")
  ( cd skills && zip -qr "../packages/$name.skill" "$name" -x '*.DS_Store' '*.zip' '*.skill' )
  cp "packages/$name.skill" "packages/$name.zip"
  # A copy inside the skill folder itself, for a one-click download from the
  # repo tree. Excluded from the archive above so a rebuild never nests it.

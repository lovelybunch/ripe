...
Seven ship: **sdr** (sales development), **rfp-ddq** (security and diligence
questionnaires), **marketer**, **support**, **operations**, **engineer** and
**people** (hiring managers). One line on each is enough; the role file has
the rest.
...
from it. Both are scored the same way for every role:

- **The draft** — the role file names one artifact and its slots. Draft it from
  found context only, marking every slot with one of the four verdicts from
  Rule 5. Headline the **invented count** (conflicted counts as invented).
  Score: 4 = none · 3 = one · 2 = two · 1 = three or more · 0 = couldn't attempt.
- **The ten questions** — answer each from found context only; verdicts
  **cited** / **uncertain** / **can't answer**. Headline the **cited count**.
  Score: 4 = 9–10 · 3 = 7–8 · 2 = 4–6 · 1 = 1–3 · 0 = none. Every "can't
  answer" becomes a plan item.

Conflicted and invented score the same and fix completely differently. Invented
means someone writes something. Conflicted means someone decides, and no amount
of writing helps until they do. Say which in the plan.
...
One short file per role in `references/roles/`, about twenty-five lines:

1. **The job, in one line** — what this person has to do from context.
2. **Six dimensions** — one line each. Add a cap only where one bites
   ("conflicting pricing caps this at 2").
3. **The draft** — the one artifact this role actually produces, and its five or
   six slots.
4. **The ten questions** — plain questions, each tagged with the id of the
   matching question in `ripe`'s `roles/<role>.md`, so a map and a measurement
   of the same team line up. Ten is a convention; an id may appear twice.

Nothing else. The scales, the verdicts and the plan structure are here in this
file and apply to every role. Adding a file changes what **Ask** offers, since
that step enumerates this directory — add roles deliberately, and keep anything
internal out of it.

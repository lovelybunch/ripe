...
conversationally). They keep separate role files but share question ids, so their numbers line up; `ripe roles --check` enforces it.

## The two skills

Both ship in this package, and as one-click bundles you can import into Claude
Desktop or Claude.ai (Settings → Skills → Add → upload the file). The bundles are
committed under [`packages/`](packages/) too, for a direct download:
...
| **`marketer`** | marketing | positioning and retired claims, audience, voice, competitors, campaign results, cleared proof, the calendar |
| **`support`** | support and success | escalation path, known issues, account history, the approved FAQ, SLAs, refunds, QBR metrics, roadmap promises |
| **`operations`** | operations | recurring processes and owners, vendors and renewals, onboarding, the decision log, admin access, incidents, policies |
| **`engineer`** | engineering | architecture, local setup, the path to production, standards, technical decisions, on-call, secrets |
| **`people`** | hiring managers, HR | the hiring process, levels and comp, interview rubric, onboarding, policies, reviews, values, the org |
| `smoke` | anyone | two questions — a cheap plumbing check |

Adding a role is one markdown file: plain-English questions, each with the two to five facts an answer must cover. `ripe roles` lists them all.
